...
import com.example.lab7.utilities.CustomEmptyTextWatcher
import com.example.lab7.viewmodels.EditCardViewModel
import com.example.lab7.utilities.Failed
import com.example.lab7.R
import com.example.lab7.utilities.Success
import com.example.lab7.utilities.bitmap
import com.example.lab7.databinding.FragmentEditCardBinding

class EditCardFragment : Fragment() {
    private var _binding: FragmentEditCardBinding? = null
    private val binding get() = _binding!!
    private val args by navArgs<com.example.lab7.fragments.EditCardFragmentArgs>()
                ...
                status.observe(viewLifecycleOwner) {
                    ...
                    when (it) {
                        ...
                        is Success -> if (!viewModel.checkIfNewCard()) {
                            val navAction =
                                com.example.lab7.fragments.EditCardFragmentDirections.actionEditCardFragmentToSeeCardFragment(
                                    cardId
                                )
                            findNavController().navigate(navAction)
                        } else {
                            val navAction =
                                com.example.lab7.fragments.EditCardFragmentDirections.actionEditCardFragmentToListCardFragment()
                            findNavController().navigate(navAction)
                        }
                    }
                    it.isProcessed = true
